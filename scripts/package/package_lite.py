...
def write_portable_helpers(target_dir: Path) -> None:
    ...
    install_bat.write_text(
        ...
        "$shortcut=(New-Object -ComObject WScript.Shell).CreateShortcut((Join-Path $desktop 'PF Searcher Lite.lnk')); "
        ...
        "$shortcut.Save()\"\n"
        "echo Done. Starting PF Searcher Lite ...\n"
...
def portable_readme_text() -> str:
    return (
        "PF Searcher Lite 使用说明\n"
        "=========================\n\n"
        "这是 PF_RAG 的精简分发版，面向本地浏览和查询。它不包含数据提取脚本、CHM 原始文件、"
        "向量索引、BM25 索引、LLM 与 Embedding 相关依赖，因此体积比完整 RAG 版本更小。\n\n"
        "一、快速使用\n"
        "1. 解压整个压缩包，不要只单独拷贝 exe。\n"
        "2. 双击 start.bat 或 PFSearcherLite.exe 直接运行。\n"
        "3. 程序会启动一个本地服务，并自动打开浏览器页面。\n"
        "4. 如果浏览器没有自动打开，请查看窗口中显示的地址，通常形如：\n"
        "   http://localhost:8000/web/\n\n"
        "二、安装到本机\n"
        "1. 双击 install.bat。\n"
        "2. 程序会复制到：%LOCALAPPDATA%\\PFSearcherLite\n"
        "3. 安装脚本会在桌面创建“PF Searcher Lite”快捷方式。\n\n"
        "三、包含的功能\n"
        ...
        "- data/：少量来源元数据。\n\n"
        "六、常见问题\n"
        "1. 页面打不开：确认 PFSearcherLite.exe 仍在运行，并访问窗口中显示的 localhost 地址。\n"
